...
exports.create = (req, res) => {
    ...
    Departamento.create(departamentos)
        .then(data => {
            res.send(data);
        })
        .catch(err => {
            res.status(500).send({
                message:
                    err.message || "Some error occurred while creating the departamentos."
            });
        });
};
    ...
    Departamento.update(req.body, {
        ...
        .then(num => {
            if (num == 1) {
                ...
                res.send({
                    message: `Cannot update departamento with id=${id}. Maybe Client was not found or req.body is empty!`
                });
            }
        })
        .catch(err => {
    ...
    Departamento.destroy({
        ...
        .then(num => {
            if (num == 1) {
                ...
                res.send({
                    message: `Cannot delete Departamento with id=${id}. El Departamento no fue encontado!`
                });
            }
        })
        .catch(err => {
...
exports.deleteAll = (req, res) => {
    Departamento.destroy({
        ...
        .then(nums => {
            res.send({ message: `${nums} Departamento were deleted successfully!` });
        })
        .catch(err => {
            res.status(500).send({
                message:
                    err.message || "Some error occurred while removing all Departamento."
            });
        });
};
